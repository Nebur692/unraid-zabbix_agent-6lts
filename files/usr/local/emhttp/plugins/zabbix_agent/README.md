...
- Added a Settings page (see Settings > Utilities > Zabbix Agent) for the most common options. Saving it
  once makes zabbix_agentd.custom.conf generated by the page from then on. Includes two live log viewers
  (Unraid's native live terminal, same as Docker/VMs/NUT): one that works immediately from syslog, and an
  opt-in one backed by a dedicated log file. Clicking the plugin from the Plugins tab now opens this page.
...
- Añadida una página de Settings (ver Settings > Utilities > Zabbix Agent) con las opciones más comunes. Al
  guardarla una vez, zabbix_agentd.custom.conf pasa a generarse desde esa página. Incluye dos visores de logs
  en vivo (terminal en vivo nativo de Unraid, igual que Docker/VMs/NUT): uno que funciona ya mismo desde
  syslog, y otro opcional respaldado por un fichero de log propio. Al hacer clic en el plugin desde la
  pestaña Plugins ahora se abre esta página.
